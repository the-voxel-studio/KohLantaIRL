...
class AlliancesCog(commands.Cog):
    ...
    async def rename(self, interaction: discord.Interaction, nouveau_nom: str):
        ...
        embed = discord.Embed(
            title=f":robot: Alliance renommée :moyai:", color=COLOR_GREEN
        )
    ...
    async def ajouter(self, interaction: discord.Interaction, user: discord.Member):
        ...
        if not await self.id_renamed(interaction):
            ...
            embed = discord.Embed(
                ...
                description=f":warning: **Vous devez tout d'abord renommer l'alliance !**",
                color=COLOR_ORANGE,
            )
            embed.add_field(
                name="Comment faire ?",
            ...
            embed = discord.Embed(
                title=f":robot: Nouveau membre :moyai:",
            ...
            embed = discord.Embed(
                title=f":robot: Action impossible :moyai:",
                description=f":warning: Le joueur spécifié a été éliminé lors d'un vote, il est donc impossible de l'ajouter à une alliance.",
                color=COLOR_ORANGE,
            )
            await interaction.followup.send(embed=embed)
