...
async def new_alliance(interaction: discord.Interaction):
    ...
    if not player.alive:
        ...
        embed=discord.Embed(title=f":robot: Nouvelle alliance :moyai:", description=f":white_check_mark: L'alliance {channel_name} a bien été créée : rendez-vous ici <#{new_text_channel.id}> pour la renommer !", color=COLOR_GREEN)
        embed.set_footer(text="Pas d'inquiétude, seul toi peut voir ce message. Aucun autre joueur, pas même les administrateurs, voient que tu as créé une alliance.")
        await interaction.followup.send(embed=embed,ephemeral=True)
